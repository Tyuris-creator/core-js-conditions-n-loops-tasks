...
function isContainNumber(num, digit) {
  let loweringNum = num;
  while (loweringNum > 0) {
    const lastDigit = loweringNum % 10;
    if (lastDigit === digit) {
      return true;
    }
    loweringNum = Math.floor(loweringNum / 10);
  }
  return false;
}
...
function getSpiralMatrix(size) {
  ...
  while (startRow <= endRow && startCol <= endCol) {
    for (let i = startCol; i <= endCol; i += 1) {
      matrix[startRow][i] = currNum;
      currNum += 1;
    }
    startRow += 1;

    for (let j = startRow; j <= endRow; j += 1) {
      matrix[j][endCol] = currNum;
      currNum += 1;
    }
    endCol -= 1;
...
function shuffleChar(str, iterations) {
  ...
  let oddStr = '';
  let evenStr = '';
  const state = { value: str, period: 0 };
  ...
  const left = iterations % state.period;
